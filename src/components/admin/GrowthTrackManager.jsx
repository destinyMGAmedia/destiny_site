...
import { Plus, Video, FileText, HelpCircle, Trash2, Save, Loader2, Pencil, X } from 'lucide-react'
  ...
  async function handleSaveContent() {
    ...
    try {
      const res = await fetch('/api/admin/growth', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'EDIT_CONTENT', ...editingContent })
      })
      if (res.ok) {
        fetchStages()
  ...
  async function handleSaveQuestion() {
    ...
    try {
      const res = await fetch('/api/admin/growth', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'EDIT_QUESTION', ...editingQuestion })
      })
      if (res.ok) {
        fetchStages()
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
      ...
      <div className="space-y-2">
        ...
        {stages.map(stage => (
          <button
            key={stage.id}
            onClick={() => { setActiveStageId(stage.id); setEditingContent(null); setEditingQuestion(null) }}
                ...
                {activeStage.contents.map(item => (
                  <div key={item.id}>
                    {editingContent?.id === item.id ? (
                      <div className="border-2 border-purple-200 rounded-xl p-6 bg-purple-50 space-y-4">
                        <div className="flex items-center justify-between mb-2">
                          <p className="font-bold text-sm text-purple-700">Editing Lesson</p>
                        ...
                        <button
                          ...
                          disabled={submitting || !editingContent.title}
                ...
                {activeStage.questions.map((q, idx) => (
                  <div key={q.id}>
                    {editingQuestion?.id === q.id ? (
                      <div className="border-2 border-purple-200 rounded-xl p-6 bg-purple-50 space-y-4">
                        <div className="flex items-center justify-between mb-2">
                          <p className="font-bold text-sm text-purple-700">Editing Question {idx + 1}</p>
                            ...
                            <div key={i} className="flex gap-2">
                              <input
                                ...
                                onChange={e => {
                                  ...
                                  const updated = { ...editingQuestion, options: opts }
                                  if (editingQuestion.correctAnswer === opt) updated.correctAnswer = e.target.value
                        ...
                        <button
                          ...
                          disabled={submitting || !editingQuestion.question || !editingQuestion.correctAnswer}
                          className="btn-primary btn-sm w-full justify-center"
                        >
                          <Save size={14} /> Save Changes
                        </button>
                      ...
                      <div className="p-6 bg-gray-50 rounded-xl relative">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <button
                              onClick={() => setEditingQuestion({ ...q, options: Array.isArray(q.options) ? q.options : [] })}
                              className="p-2 text-gray-400 hover:text-purple-600 rounded-lg hover:bg-purple-50 transition-colors"
                              title="Edit"
                            >
                              <Pencil size={15} />
